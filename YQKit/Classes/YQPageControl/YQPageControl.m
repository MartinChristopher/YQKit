...
-(void)initialize{
    ...
    _PageControlContentMode = YQPageControlContentModeCenter;
    _marginSpacing = 12;
    _numberOfPages=0;
    _currentPage=0;
...
- (void)setPageControlContentMode:(YQPageControlContentMode)PageControlContentMode {
...
-(void)setCurrentPage:(NSInteger)currentPage{
    ...
    if([self.delegate respondsToSelector:@selector(YQPageControlClick:index:)])
    {
        [self.delegate YQPageControlClick:self index:currentPage];
    }
...
-(void)setPageControlStyle:(YQPageControlStyle)PageControlStyle{
...
-(void)createPointView{
    ...
    switch (_PageControlStyle) {
        case YQPageControlStyelDefault:
        {
            [self CreatDotWithSize:_controlSize];
        }
            break;
        case YQPageControlStyelRectangle:
        {
            [self CreatDotWithSize:CGSizeMake(15, 2)];
        }
            break;
        case YQPageControlStyelDotAndRectangle:
        {
...
-(void)CreatDotWithSize:(CGSize)DotSize{
    ...
    if(self.frame.size.width<mainWidth){
        ...
        if (_PageControlContentMode == YQPageControlContentModeLeft && self.frame.size.width - _marginSpacing * 2 > mainWidth) {
            startX = _marginSpacing;
        }else if (_PageControlContentMode == YQPageControlContentModeRight && self.frame.size.width - _marginSpacing * 2 > mainWidth) {
